  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Geospatial Map Visualizer (Futuristic SVG layout) */}
        <div className="bg-slate-900/40 border border-white/5 backdrop-blur-md rounded-[24px] p-6 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-200 tracking-tight">Geospatial Hotspots</h3>
              <span className="text-[10px] font-bold text-rose-400 bg-rose-500/10 border border-rose-500/20 px-2 py-0.5 rounded-full animate-pulse">4 active zones</span>
            </div>
            <p className="text-xs text-slate-500">Live city coordinate alert nodes</p>
          </div>

          {/* Futuristic Mini Map SVG representation */}
          <div className="h-44 my-3 bg-slate-950/60 rounded-2xl border border-white/5 relative flex items-center justify-center overflow-hidden">
            {/* Background grid */}
            ...
            {/* SVG Roads & Node Hotspots */}
            <svg className="w-full h-full text-slate-800/40 opacity-70 p-4" viewBox="0 0 200 100" fill="none" xmlns="http://www.w3.org/2000/svg">
              {/* Grid map paths */}
              <path d="M10 20 H190 M10 50 H190 M10 80 H190" stroke="currentColor" strokeWidth="0.8" strokeDasharray="3 3" />
              <path d="M40 5 V95 M100 5 V95 M160 5 V95" stroke="currentColor" strokeWidth="0.8" strokeDasharray="3 3" />
              
              {/* Primary roads */}
              <path d="M25 10 L175 90 M25 90 L175 10" stroke="#334155" strokeWidth="1.5" />
              <path d="M100 0 V100" stroke="#334155" strokeWidth="2" />
              <path d="M0 50 H200" stroke="#334155" strokeWidth="2" />
              
              {/* Hotspot node animations */}
              <circle cx="50" cy="30" r="4" fill="#ef4444" className="animate-ping" style={{ transformOrigin: '50px 30px' }} />
              <circle cx="50" cy="30" r="3" fill="#ef4444" />
              
              <circle cx="130" cy="70" r="4" fill="#f59e0b" className="animate-ping" style={{ transformOrigin: '130px 70px' }} />
              <circle cx="130" cy="70" r="3" fill="#f59e0b" />
              
              <circle cx="100" cy="50" r="6" fill="#ef4444" className="animate-ping" style={{ transformOrigin: '100px 50px' }} />
              <circle cx="100" cy="50" r="4" fill="#ef4444" />
              
              <circle cx="160" cy="20" r="4" fill="#6366f1" className="animate-ping" style={{ transformOrigin: '160px 20px' }} />
              <circle cx="160" cy="20" r="3" fill="#6366f1" />
            </svg>

            {/* Custom map layout overlay tags */}
            <div className="absolute top-2 left-2 bg-slate-900/90 border border-white/10 px-2 py-0.5 rounded text-[8px] font-black text-slate-400">SECTOR-4</div>
            <div className="absolute bottom-2 right-2 bg-slate-900/90 border border-white/10 px-2 py-0.5 rounded text-[8px] font-black text-slate-400">BLOCK-B</div>
          </div>

          <button className="w-full flex items-center justify-center space-x-1 py-2 rounded-xl bg-slate-900 border border-white/5 hover:border-white/15 text-xs font-bold text-slate-300 hover:text-white transition-all cursor-pointer">
            <MapPin className="w-3.5 h-3.5 text-indigo-400" />
            <span>Open Spatial Analyzer Map →</span>
          </button>
        </div>
            ...
            <tbody className="divide-y divide-white/5 text-xs font-medium">
              {complaints.length > 0 ? complaints.map((complaint) => (
                <tr key={complaint.id} className="hover:bg-slate-900/25 transition-colors group">
                  <td className="p-4">
                    <div>
                      <p className="text-slate-200 font-bold group-hover:text-indigo-400 transition-colors">{complaint.title}</p>
                  ...
                  <td className="p-4 text-right">
                    <select 
                      className="bg-slate-950 border border-white/5 hover:border-white/15 text-slate-300 text-xs rounded-xl focus:ring-1 focus:ring-indigo-500 block p-2 w-full max-w-[130px] ml-auto outline-none cursor-pointer transition-all"
